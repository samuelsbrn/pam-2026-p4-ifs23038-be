...
fun Application.configureRouting() {
    ...
    val motorService: MotorService by inject()
    ...
    routing {
        ...
        route("/motors") {
            get {
                motorService.getAllMotors(call)
            }
            post {
                motorService.createMotor(call)
            }
            get("/{id}") {
                motorService.getMotorById(call)
            }
            put("/{id}") {
                motorService.updateMotor(call)
            }
            delete("/{id}") {
                motorService.deleteMotor(call)
            }

            get("/{id}/image") {
                motorService.getMotorImage(call)
            }
        }
